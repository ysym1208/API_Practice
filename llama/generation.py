...
from llama.model import YourModelClass  # 상대 경로로 임포트
from llama.tokenizer import YourTokenizerClass  # 상대 경로로 임포트
...
def generate_feedback(inputs, device):
    # 모델을 디바이스로 이동
    model.to(device)
    ...
    output_tokens = outputs[0].argmax(dim=-1).tolist()
